...
class SpatialGrid[T]:
    ...
    종·엔티티에 의존하지 않는 일반 자료구조다(임의 객체 T + 위치). distance 이내 항목이
    ...
    def _cell(self, loc: pygame.Vector2) -> tuple[int, int]:
        cs = self._cs
        return (int(loc[0] // cs), int(loc[1] // cs))

    def insert(self, item: T, loc: pygame.Vector2) -> None:
        self._cells.setdefault(self._cell(loc), []).append(item)

    def nearby(self, loc: pygame.Vector2, radius: float) -> list[T]:
